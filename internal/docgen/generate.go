...
func writeFile(path, binName string, cmd *cobra.Command) error {
	...
	writeGroupFile(&content, binName, cmd, true)
	return writeMarkdown(path, content.String())
}

func writeStandalonesFile(path, binName string, cmds []*cobra.Command) error {
	var content strings.Builder

	fmt.Fprintln(&content, "<!-- auto-generated by cmd/docgen — do not edit -->")
	fmt.Fprintln(&content)
	fmt.Fprintln(&content, "# Other Commands")
	fmt.Fprintln(&content)
	...
	for i, cmd := range cmds {
		...
		writeGroupFile(&content, binName, cmd, false)
	}
	return writeMarkdown(path, content.String())
}
